  ...
  hadronizerType_(0)
{
  ...
  //runLeptons_ = iConfig.getParameter<bool>("runLeptons");
  ...
  // lepton information
  //-------------------------------------
  if ( true ) LeptonInfo.RegisterTree(smalltree);
...
//$$
  ...
  //------------------------------------------------------
    ...
    for (size_t i = 0; i<prunedGenParticles->size(); ++i) {
      const GenParticle & pGenIt = (*prunedGenParticles)[i];
    
	    LeptonInfo.GenPart_pt[LeptonInfo.nGenPart] = pGenIt.pt();
	    LeptonInfo.GenPart_phi[LeptonInfo.nGenPart] = pGenIt.phi();
	    LeptonInfo.GenPart_eta[LeptonInfo.nGenPart] = pGenIt.eta();	
	    LeptonInfo.GenPart_mass[LeptonInfo.nGenPart] = pGenIt.mass();
	    LeptonInfo.GenPart_pdgid[LeptonInfo.nGenPart] = pGenIt.pdgId();
      LeptonInfo.GenPart_status[LeptonInfo.nGenPart] = pGenIt.status();
      int idx = -1;
      for( reco::GenParticleCollection::const_iterator mIt = prunedGenParticles->begin(); mIt != prunedGenParticles->end(); ++mIt ) {
        if( pGenIt.mother() == &(*mIt) ) {
          idx = std::distance(prunedGenParticles->begin(),mIt);
          break;
        }
      }
      LeptonInfo.GenPart_mother[LeptonInfo.nGenPart] = idx;
      LeptonInfo.GenPart_vertX[LeptonInfo.nGenPart] = pGenIt.vx();
      LeptonInfo.GenPart_vertY[LeptonInfo.nGenPart] = pGenIt.vy();
      LeptonInfo.GenPart_vertZ[LeptonInfo.nGenPart] = pGenIt.vz();
	    ++LeptonInfo.nGenPart;	
      
     /* std::cout << "****************************" << endl;
      std::cout << "pdgID: " << LeptonInfo.GenPart_pdgid[LeptonInfo.nGenPart] <<endl;
      std::cout << "index: " << LeptonInfo.nGenPart << endl;
      std::cout << "vertex x: " << LeptonInfo.GenPart_vertX[LeptonInfo.nGenPart] << endl;
      std::cout << "vertex y: " << LeptonInfo.GenPart_vertY[LeptonInfo.nGenPart] << endl;
      std::cout << "vertex z: " << LeptonInfo.GenPart_vertZ[LeptonInfo.nGenPart] << endl; */
    }
...
//------------------------------------------------------
   ...
   iEvent.getByLabel(patMuonCollectionName_,patMuonsHandle);
   for( std::vector<pat::Muon>::const_iterator it = patMuonsHandle->begin(); it != patMuonsHandle->end(); ++it ){
      LeptonInfo.Mu_trkIso[LeptonInfo.nMu] = it->trackIso();
      LeptonInfo.Mu_trackerIsoSumPT[LeptonInfo.nMu] = it->isolationR03().sumPt;
      LeptonInfo.Mu_ecalIso[LeptonInfo.nMu] = it->ecalIso();	
      LeptonInfo.Mu_hcalIso[LeptonInfo.nMu] = it->hcalIso();
      LeptonInfo.Mu_hoIso[LeptonInfo.nMu] = it->isolationR03().hoEt;
      LeptonInfo.Mu_ecalVetoIso[LeptonInfo.nMu] = it->isolationR03().emVetoEt;
      LeptonInfo.Mu_hcalVetoIso[LeptonInfo.nMu] = it->isolationR03().hadVetoEt;
      LeptonInfo.Mu_pfisor03chargedhadron[LeptonInfo.nMu] = it->pfIsolationR03().sumChargedHadronPt;
      LeptonInfo.Mu_pfisor03chargedparticle[LeptonInfo.nMu] = it->pfIsolationR03().sumChargedParticlePt;
      LeptonInfo.Mu_pfisor03neutralhadron[LeptonInfo.nMu] = it->pfIsolationR03().sumNeutralHadronEt;
      LeptonInfo.Mu_pfisor03photon[LeptonInfo.nMu] = it->pfIsolationR03().sumPhotonEt;
      LeptonInfo.Mu_pfisor03neutralhadronht[LeptonInfo.nMu] = it->pfIsolationR03().sumNeutralHadronEtHighThreshold;
      LeptonInfo.Mu_pfisor03photonht[LeptonInfo.nMu] = it->pfIsolationR03().sumPhotonEtHighThreshold;
      LeptonInfo.Mu_pfisor03pu[LeptonInfo.nMu] = it->pfIsolationR03().sumPUPt;
      LeptonInfo.Mu_pfisor04chargedhadron[LeptonInfo.nMu] = it->pfIsolationR04().sumChargedHadronPt;
      LeptonInfo.Mu_pfisor04chargedparticle[LeptonInfo.nMu] = it->pfIsolationR04().sumChargedParticlePt;
      LeptonInfo.Mu_pfisor04neutralhadron[LeptonInfo.nMu] = it->pfIsolationR04().sumNeutralHadronEt;
      LeptonInfo.Mu_pfisor04photon[LeptonInfo.nMu] = it->pfIsolationR04().sumPhotonEt;
      LeptonInfo.Mu_pfisor04neutralhadronht[LeptonInfo.nMu] = it->pfIsolationR04().sumNeutralHadronEtHighThreshold;
      LeptonInfo.Mu_pfisor04photonht[LeptonInfo.nMu] = it->pfIsolationR04().sumPhotonEtHighThreshold;
      LeptonInfo.Mu_pfisor04pu[LeptonInfo.nMu] = it->pfIsolationR04().sumPUPt;
      LeptonInfo.Mu_isPF[LeptonInfo.nMu] = it->isPFMuon();
      LeptonInfo.Mu_isGlobal[LeptonInfo.nMu] = it->isGlobalMuon();
      LeptonInfo.Mu_isTracker[LeptonInfo.nMu] = it->isTrackerMuon();
      LeptonInfo.Mu_nTrackLayers[LeptonInfo.nMu] = (it->isTrackerMuon()? it->innerTrack()->hitPattern().trackerLayersWithMeasurement(): -99);
      LeptonInfo.Mu_nPixelLayers[LeptonInfo.nMu] = (it->isTrackerMuon()? it->innerTrack()->hitPattern().pixelLayersWithMeasurement(): -99);
      LeptonInfo.Mu_nPixelHits[LeptonInfo.nMu] = (it->isTrackerMuon()? it->innerTrack()->hitPattern().numberOfValidPixelHits(): -99);
      LeptonInfo.Mu_nValidHits[LeptonInfo.nMu] = (it->isGlobalMuon()? it->globalTrack()->hitPattern().numberOfValidMuonHits(): -99);
      LeptonInfo.Mu_nMatchedStations[LeptonInfo.nMu] = it->numberOfMatchedStations();
      LeptonInfo.Mu_innerTrack_normChi2[LeptonInfo.nMu] = (it->isTrackerMuon()? it->innerTrack()->normalizedChi2():-99);
      LeptonInfo.Mu_innerTrack_dxyVertPos[LeptonInfo.nMu] = (it->isTrackerMuon()? fabs(it->innerTrack()->dxy((*primaryVertex)[0].position())): -99);
      LeptonInfo.Mu_innerTrack_dzVertPos[LeptonInfo.nMu] = (it->isTrackerMuon()? fabs(it->innerTrack()->dz((*primaryVertex)[0].position())): -99);
      LeptonInfo.Mu_bestTrack_dzVertPos[LeptonInfo.nMu] = fabs(it->muonBestTrack()->dz((*primaryVertex)[0].position()));
      LeptonInfo.Mu_bestTrack_dxyVertPos[LeptonInfo.nMu] = fabs(it->muonBestTrack()->dxy((*primaryVertex)[0].position()));
      LeptonInfo.Mu_globalTrack_normChi2[LeptonInfo.nMu] = (it->isGlobalMuon()? it->globalTrack()->normalizedChi2():-99);
      LeptonInfo.Mu_IP3D[LeptonInfo.nMu]       = it->dB(pat::Muon::PV3D);
      LeptonInfo.Mu_IP3Der[LeptonInfo.nMu]    = it->edB(pat::Muon::PV3D);
      LeptonInfo.Mu_IP2D[LeptonInfo.nMu]     = it->dB(pat::Muon::PV2D);
      LeptonInfo.Mu_IP2Der[LeptonInfo.nMu]  = it->edB(pat::Muon::PV2D); 
      LeptonInfo.Mu_pt[LeptonInfo.nMu] = it->pt();
      LeptonInfo.Mu_phi[LeptonInfo.nMu] = it->phi();
      LeptonInfo.Mu_eta[LeptonInfo.nMu] = it->eta();
      LeptonInfo.Mu_mass[LeptonInfo.nMu] = it->mass();
      
//      std::cout << "Muon 3D IP = " << LeptonInfo.Mu_IP3D[LeptonInfo.nMu] << std::endl; 
